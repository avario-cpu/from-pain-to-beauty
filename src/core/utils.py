...
class LockFileManager:
    ...
    def __init__(self, script_name: str, logger: Logger = None):
        self.filename = script_name  # name of script instantiating the class
        self.lock_dir = LOCK_DIR
        self.lock_file_path = os.path.join(self.lock_dir,
                                           f"{self.filename}.lock")
    ...
    def remove_lock_file(self):
        self.logger.debug(f"Attempting to remove lock file for"
                          f" {self.filename}")
        try:
            ...
            self.logger.debug(
                f"Lock file path: {self.lock_file_path} does not exist.")
